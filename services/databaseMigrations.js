...
export const migrations = {
        ...
        try {
            ...
            switch (targetVersion) {
                case 1:
                    ...
                    await databaseService.db.execAsync('DROP TABLE IF EXISTS product_images');
                    await databaseService.db.execAsync('DROP TABLE IF EXISTS wishlist');
                    await databaseService.db.execAsync('DROP TABLE IF EXISTS notifications');
                    await databaseService.db.execAsync('DROP TABLE IF EXISTS user_preferences');
                    await databaseService.db.execAsync('DROP TABLE IF EXISTS product_variants');
                    await databaseService.db.execAsync('DROP TABLE IF EXISTS coupons');
                    await databaseService.db.execAsync('DROP TABLE IF EXISTS order_coupons');
                    break;
                case 2:
    ...
    async getCurrentVersion() {
        ...
        `);
                ...
                await databaseService.db.execAsync('INSERT INTO db_version (version) VALUES (1)');
                return 1;
            }

            const result = await databaseService.db.getFirstAsync('SELECT MAX(version) as version FROM db_version');
    ...
    async updateVersion(version) {
        try {
            await databaseService.db.execAsync('INSERT INTO db_version (version) VALUES (?)', [version]);
            return true;
        } catch (error) {
